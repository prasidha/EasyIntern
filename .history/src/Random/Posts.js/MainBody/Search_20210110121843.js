 ...
 const {setGetData,getData} = useAuth()
  ...
  const searchIntern = (e) => {
      const search = getData.filter((value) =>{
       if(getData.internPost.internTitle.toLowerCase().indexOf(e.target.value.toLowerCase()) != -1){
         return  value
       }
  ...
  return (
    ...
    <div className={classes.root}>
      <Grid container spacing={3}>
        <Grid item xs={12} >
        <h1 style={{margin:'4rem',textAlign:'center'}}>Find an Intern you love</h1>
          <Paper className={classes.paper}>
            <div className={classes.Search}>
               <input type="text" placeholder=" search intern vacancy.." className={classes.input} onChange={searchIntern} autoComplete="off"/>
   ...
   <div style={{margin:'5rem',width:'95%'}}>
     <h1 style={{textAlign:'center'}}> Top  Interns  available</h1>
     {duplicateData.length === 0 && <h1 style={{textAlign:'center'}}><CircularProgress/></h1>}
