...
{ userData.isStudent === false && <Button color="primary" variant="contained"  onClick={(event)=>setAnchorE2(event.currentTarget)} className={classes.menuButton}>{userData.companyName} <AccountCircleIcon/></Button>}
<Menu
    anchorE2={anchorE2}
...
className={classes.Menu}
>
 <Link to="" style={{textDecoration:'none'}}> <MenuItem onClick={()=>{setA(false)}}>Applicants</MenuItem> </Link>
 <Link to="/postintern" style={{textDecoration:'none'}}> <MenuItem onClick={handleClose}>Post an Intern</MenuItem>  </Link>
  <MenuItem onClick={()=>{setOpen(false)}}><Button onClick={onLogOut}>Logout</Button></MenuItem>
</Menu>
