...
{  <Link to="/companydashboard" style={{textDecoration:'none'}}> <Button color="primary" variant="contained" className={classes.menuButton}><AccountCircleIcon/></Button></Link>}

{userData.isStudent === false && <Button  onClick={()=>{setOpen(true)}}>
...
>
 <Link> <MenuItem onClose={()=>{setOpen(false)}}>Applicants</MenuItem> </Link>
 <Link> <MenuItem onClose={()=>{setOpen(false)}}>Post an Intern</MenuItem>  </Link>
 <Link> <MenuItem onClose={()=>{setOpen(false)}}>Logout</MenuItem> </Link>
</Menu>
        </Toolbar>
      </AppBar>
    </div>
