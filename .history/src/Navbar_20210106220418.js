...
{ userData.isStudent === false && <Button color="primary" variant="contained"  onClick={()=>{setOpen(true)}} className={classes.menuButton}>{userData.companyName}</Button>}
<Menu
open={open}
onClose={()=>{setOpen(false)}}
className={classes.Menu1}
>
 <Link to="" style={{textDecoration:'none'}}> <MenuItem onClose={()=>{setOpen(false)}}>Applicants</MenuItem> </Link>
 <Link to="/postintern" style={{textDecoration:'none'}}> <MenuItem onClose={()=>{setOpen(false)}}>Post an Intern</MenuItem>  </Link>
  <MenuItem onClose={()=>{setOpen(false)}} onClick={onHandleLogout}>Logout</MenuItem>
</Menu>
