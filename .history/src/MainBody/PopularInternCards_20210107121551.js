...
export default function RecipeReviewCard({dataa,id}) {
  const classes = useStyles();
  const [expanded, setExpanded] = React.useState(false);
  const{ userData ,currentUser} = useAuth()
  // const[fav,setFav]=React.useState(1)
  const[error,setError]=React.useState('')
  const[applyPost,setApplyPost] =React.useState([])
  ...
  // console.log(dataa.cur_id,"id....")
    ...
    useEffect(() => {
      ...
      .then(snapshot => {
        ...
        setApplyPost({applyPost})
        console.log(applyPost,"appl")
      })
    }, [])
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        ...
        <Typography paragraph>
         {dataa.internPost.internTitle}
          ...
          {applyPost && applyPost.map( apply => 
            
            {return 
              
              {  <Button color="primary" variant="contained" style={{width:'100%'}} onClick={onApplyPost}>apply now</Button> :"" }
