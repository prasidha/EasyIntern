...
export default function RecipeReviewCard({dataa}) {
  ...
  const[applyPosts,setapplyPosts] =React.useState("apply now")
  ...
  const addToFav = () => {
    db.collection("favourite").doc()
  }
  const applyPost = () => {
    db.collection("Notification").doc(userData.company_id).set({
                id:dataa.internPost.id,
    })
    setapplyPosts("applied");
  }
  return (
  ...
  { userData?.isStudent === true?  <Button color="primary" variant="contained" style={{width:'100%'}} onClick={applyPost}>{applyPosts ? "applied" :'apply now'} </Button> :"" }
