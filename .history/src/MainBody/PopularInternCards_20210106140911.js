...
export default function RecipeReviewCard({dataa}) {
  ...
  useEffect(() => {
    ...
    db.collection("notification").get().then((querySnapshot)=> {
      ...
      setApplyPosts(querySnapshot.docs.map(doc => (doc.data())))
    })
  
}, [true])
console.log("dddd",applyPosts)
  
const isApplied = applyPosts.map(d =>{
  return d.appliedPost ;
}
)
console.log(isApplied.includes(appliedPost),"isApplied....")
  return (
