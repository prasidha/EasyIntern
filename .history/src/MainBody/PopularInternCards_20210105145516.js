...
export default function RecipeReviewCard({dataa}) {
  ...
  const[applyPosts,setapplyPosts] =React.useState(false)
  ...
  const addToFav = () => {
    ...
    db.collection("favourite").doc(currentUser.uid).update({
         ...
         fav_:firebase.firestore.FieldValue.arrayUnion(dataa.cur_id) ,
  ...
  const onApplyPost = () => {
    ...
    let id1=userData.student_id;
    let id= dataa.cur_id;
    let data=dataa.internPost;

    db.collection("notification").doc(currentUser.uid).update({
            appliedBy:{id:userData.student_id,userData:userData},
            appliedPost: firebase.firestore.FieldValue.arrayUnion({id:dataa.cur_id,internPost:dataa.internPost})      
    })
    .then(function() {
      console.log("Document successfully written!");
  })
  ...
  { userData?.isStudent === true?  <Button color="primary" variant="contained" style={{width:'100%'}} onClick={onApplyPost}>apply</Button> :"" }
