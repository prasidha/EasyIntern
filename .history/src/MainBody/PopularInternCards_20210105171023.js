...
export default function RecipeReviewCard({dataa}) {
  ...
  const onApplyPost = () => {
    ...
    db.collection("notification").doc().update({
  ...
  useEffect(() => {
   db.collection("notification").doc().get().then(value=>{
  ...
  { userData?.isStudent === true?  <Button color="primary" variant="contained" style={{width:'100%'}} onClick={onApplyPost}>{appliedPost.includes(dataa.cur_id) ?"applied" : "apply"}</Button> :"" }
