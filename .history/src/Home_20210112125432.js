...
export default () => {
    ...
    const { userData ,setGetData ,getData ,currentUser} = useAuth()
  ...
  const internApply = (id) => {
    // db.collection("notification").add({
    //     appliedPost:props.data.id,
    //     // appliedBy:userData.student_id,
    //     timestamp:Date.now()
    // })
    console.log(currentUser.email,userData.email,"aplliedBy")
}
 ...
 Register as a Student <SchoolIcon/>
  ...
  <JobViewModal data={viewModal} closeModal={()=>setViewModal({})} onApplyIntern={internApply(id)}/>
